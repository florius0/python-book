...
При определении параметров можно указать их тип (что не запретит передавать аргументы других типов) через двоеточие после определения параметра через двоеточие: `parameter_defenition: type_annotation`, где `parameter_definition` и `type_annotation` – определение параметра и аннотация типа соответственно.

Мы не будем углубляться в возможности аннтоаций типов, скажем только что в качестве аннотации может использоваться имя типа (`int`, `float`, `bool`, `str`, `list` ...). Подробнее об аннотация типов можно прочитать в официальной докуметации Python по ссылке <https://docs.python.org/3/library/typing.html>.

### 5. Декораторы

Декоратором называется выражение, значение которого является функцией (строго говоря, значение должно быть вызваемым, то есть иметь метод __call__) и которое как бы оборачивает вызов функции в другую функцию.
...
func = decorator1(arg)(decorator2(func))
```

Реализуем свой декоратор, который будет печатать результат функции а так же аргументы, с которыми она была вызвана(и возвращать его, чтобы снаружи логика работы функкции не изменилась):

<!-- 
filename: chapter_7/decorators.py
-->
